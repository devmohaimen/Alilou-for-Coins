...
def get_aliexpress_product_info(product_url):
    ...
    try:
        ...
        if not product_name:
            h1 = soup.find(
                "h1", {
                    "class":
                    lambda x: x and
                    ("product-title-text" in x or "product-title" in x)
                })
            if h1:
                product_name = h1.get_text(strip=True)
        ...
        img_tag = soup.find("img",
                            {"class": lambda x: x and "magnifier--image" in x})
